...
resumir_por_programa <- function(df,periodo_objetivo) {
  df %>%
    ...
    summarise(
      n_estudiantes = n(),
      n_estudiantes_matriculados = sum(periodo_bdsaberpro == periodo_objetivo, na.rm = TRUE),
      promedio_punt_saberpro = mean(punt_global_bdsaberpro, na.rm = TRUE),
      nombre_institucion = first(nombre_institucion),
      periodo = first(año),
      .groups = "drop"
    )
...
base_saber_pro <- read_delim("data/BD/saber11_nacional_saberpro_bogota_region.csv", escape_double = FALSE, trim_ws = TRUE)
#Leer la base consolidada de los programas en bogota region
programas <- read_delim("data/SNIES_CINE_cleaned/base_cine.csv", escape_double = FALSE, trim_ws = TRUE)
...
data <- left_join(
  ...
  filter(!is.na(estu_snies_prgmacademico) & 
           !is.na(codigo_institucion) & 
...
años <- c(2023)
for (año in años){
  data_temp <- data %>% filter(anio_presentacion_sbpro == año) 
  promedios_programa <- resumir_por_programa(data_temp)
  write_csv(promedios_programa, file = paste0("data/Promedios/Saberpro/promedios_programa_", año, ".csv"))
}
