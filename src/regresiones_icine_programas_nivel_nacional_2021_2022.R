...
data <- read_delim("data/BD/icfes_cine_programas_vigencia_2021_2022_cruce_nacional.csv", escape_double = FALSE, trim_ws = TRUE)
...
guardar_valores_agregados <- function(data, periodos, niveles_cine) {
  for (nombre_periodo in names(periodos)) {
    ...
    for (nivel_cine in niveles_cine) {
      ...
      ruta_xlsx <- file.path(ruta_directorio, paste0("va_", nivel_cine, "_", nombre_periodo, "_nacionales_run18072025" , ".xlsx"))
