...
# Compara los estados de programas entre 2022 y 2023 y retorna solo los que cambiaron.
comparar_estado_programa <- function(df_2022, df_2023) {
  inner_join(
    df_2022,
    df_2023,
    by = "CÓDIGO_SNIES_DEL_PROGRAMA",
    suffix = c("_2022", "_2023")
  ) %>%
    select(
      CÓDIGO_SNIES_DEL_PROGRAMA,
      ESTADO_PROGRAMA_2022 = ESTADO_PROGRAMA_2022,
      ESTADO_PROGRAMA_2023 = ESTADO_PROGRAMA_2023
    ) %>%
    filter(ESTADO_PROGRAMA_2022 != ESTADO_PROGRAMA_2023)
}
...
cine_snies_2023 <- read_excel("data/SNIES_CINE_raw/programas_nivel_nacional_vigencia_2023.xlsx")

# Se agrega identificador del periodo vigencia del cual se descargaron los datos de la página del SNIES
cine_snies_2022 <- cine_snies_2022 %>%
  mutate(periodo_vigencia_descargado_pagina_snies = 2022)

cine_snies_2023 <- cine_snies_2023 %>%
  mutate(periodo_vigencia_descargado_pagina_snies = 2023)

#### Analizar como varian las columnas estados entre años ####
diferencias_estados_programas <- comparar_estado_programa(cine_snies_2022, cine_snies_2023)

# concatenar y quedarse con valores únicos por 'codigo_snies_del_programa'
cine_snies <- bind_rows(cine_snies_2022, cine_snies_2023) %>%
  distinct(CÓDIGO_SNIES_DEL_PROGRAMA, .keep_all = TRUE)
...
write_csv(base_cine, "data/SNIES_CINE_cleaned/base_cine_programas_vigencia_2022_2023_15072025.csv")
write_xlsx(base_cine, "data/SNIES_CINE_cleaned/base_cine_programas_vigencia_2022_2023_15072025.xlsx")
...
write_xlsx(base_cine_filtrada, "data/SNIES_CINE_cleaned/base_cine_bogota_programas_vigencia_2022_2023_15072025.xlsx")

#municipios en la base_cine_filtrada
sort(unique(base_cine_filtrada$municipio_oferta_programa))

#3.1 Cruzar informacion del icfes con la informacion del CINE, filtrando por los estados del porograma
...
write_csv(data, "data/BD/icfes_cine_programas_vigencia_2022_2023_cruce_nacional.csv")
write_xlsx(data,"data/BD/icfes_cine_programas_vigencia_2022_2023_cruce_nacional.xlsx")