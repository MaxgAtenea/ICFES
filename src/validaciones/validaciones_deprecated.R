...
icfes <- read_delim("data/BD/bd.csv", escape_double = FALSE, trim_ws = TRUE)
...
icfes_summary[(mitad + 1):nrow(icfes_summary), ] %>%
  gt() %>%
  gtsave("output/icfes_summary_parte2.png")
remove(mitad)
...
municipios_unicos <- unique(base_cine$municipio_oferta_programa)

# Comparar y contar coincidencias
coincidencias_municipios <- intersect(municipios_unicos, bogota_region)
num_coincidencias_municipios <- length(coincidencias_municipios)

# Mostrar resultado
num_coincidencias_municipios
...
base_cine <- base_cine %>%
  filter(
    ...
    codigo_del_municipio_programa==11001,
...
####Guardar la tabla
#dividr la tabla en 2
mitad <- ceiling(nrow(base_cine_summary) / 2)
# Primera mitad
base_cine_summary[1:mitad, ] %>%
  gt() %>%
  gtsave("ICFES/output/snies_summary_parte1.png")
# Segunda mitad
base_cine_summary[(mitad + 1):nrow(base_cine_summary), ] %>%
  gt() %>%
  gtsave("ICFES/output/snies_summary_parte2.png")
remove(mitad)


#Verificamos que no hayan duplicados de codigos SNIES en el df base_cine
sum(base_cine[duplicated(base_cine$codigo_snies_del_programa), ])

#Mirar codigos CINE y NBC unicos por nivel
#CINE campos amplios son 11 los reportados por el DANE
#CINE campos detallados son 121 (82+10 interdisciplinarios + 10 no clasificados en otra parte)
#CINES especificos segun documento DANE
#Son 39 (29 + 10 interdisciplinarios)
...
#Mirar NBC unicos en el dataframe base_cine
...
#Guardar los valores unicos de nbc del df base_cine
nbc_base_cine <- base_cine %>%
...
max_filas <- max(nrow(nbc_icfes), nrow(nbc_base_cine), nrow(nbc_men))

# Rellenar todos
nbc_icfes <- rellenar_na(nbc_icfes, max_filas)
nbc_base_cine <- rellenar_na(nbc_base_cine, max_filas)
nbc_men <- rellenar_na(nbc_men, max_filas)

#Dataframe con el listado de NBC de cada una de las fuentes.
#El df base_cine y el listado de nbc del SNIES no tiene el nbc llamado "normales superiores"
#El listado de NBC del SNIES no tiene el campo "Sin clasificar"
listado_nbc <- bind_cols(nbc_icfes, nbc_base_cine, nbc_men)

#Liberar memoria
remove(nbc_base_cine,nbc_icfes,nbc_men,max_filas)

############################################
#Mirar los programas cuyos snies no cruzan
############################################

#Obtenemos los codigos SNIES de los programas en el dataframe base_cine
#Tambien obtenemos el nombre del programa, el nivel academico y el nivel de formacion
programas_bdbase_cine <- base_cine %>%
  select(
  ...
  distinct() %>%
  mutate(from_base_cine = TRUE) #agrega identificador de la BD donde vienen los datos


#Remover los programas que no son de pregrado
programas_bdbase_cine <- programas_bdbase_cine %>%
...
programas <- full_join(programas_bdbase_cine, programas_bdicfes, by = c("codigo_snies"))
...
programas_matching <- programas %>%
  filter(!is.na(from_base_cine) & !is.na(from_icfes))

#Programas que no cruzaron (Llave: codigo snies del programa)
#no cruzaron 921: 890 no cruzaron del base_cine y 31 no cruzaron del icfes
programas_non_matching <- programas %>%
  filter(is.na(from_base_cine) | is.na(from_icfes))

#Reemplazar NA por False en la columna from_icfes y from_base_cine
programas_non_matching <- programas_non_matching %>%
  mutate(
    from_icfes = if_else(is.na(from_icfes), FALSE, from_icfes),
    from_base_cine = if_else(is.na(from_base_cine), FALSE, from_base_cine)
  )
...
summary_programas_non_matching <- data.frame(
  ...
  from_cine_count = sum(programas_non_matching$from_base_cine, na.rm = TRUE)
)
